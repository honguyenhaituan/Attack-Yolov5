...
from attack.attacks import attack_images, get_method_attack
from attack.utils_image import cvtOriginalImage, save_images
from attack.log import WandbLogger
...
def test(data,
    ...
    if training:  # called by train.py
        ...
        (save_dir / 'images' if save_att_images else save_dir).mkdir(parents=True, exist_ok=True)
    ...
    # Method attack
    method_attack = get_method_attack(name_attack, max_iter, epsilon, momentum, decay)
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        for si, pred in enumerate(out):
            ...
            if len(wandb_images) < log_imgs:  # and wandb_logger.current_epoch > 0:  # Check for test operation
                # if wandb_logger.current_epoch % wandb_logger.bbox_iterval == 0:
    ...
    if not training:
        ...
        if wandb_logger and wandb_logger.wandb:
            wandb_logger.log({
                'time/Speed pre-process': t[0],
                'time/Speed inference': t[1],
                'time/Speed NMS per image': t[2],
            })
